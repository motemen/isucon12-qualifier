...
// 大会ごとの課金レポートを計算する
func calculateBillingData(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string) (*BillingReport, error) {
...
func getBillingReportByCompetition(ctx context.Context, tenantID int64, competitonIDs []string) (*[]BillingReport, error) {
	...
	brs := make([]BillingReport, 0, len(br))
	for _, bill := range br {
		brs = append(brs, BillingReport{
			CompetitionID:     bill.CompetitionID,
			CompetitionTitle:  bill.CompetitionTitle,
			PlayerCount:       bill.PlayerCount,
			VisitorCount:      bill.VisitorCount,
			BillingPlayerYen:  bill.BillingPlayerYen,
			BillingVisitorYen: bill.BillingPlayerYen,
			BillingYen:        bill.BillingYen,
		})
	}
...
func competitionFinishHandler(c echo.Context) error {
	...
	bills, err := calculateBillingData(ctx, tenantDB, v.tenantID, id)
	if err != nil {
		return fmt.Errorf("error calculateBillingData: %w", err)
	}
